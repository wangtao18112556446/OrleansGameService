...
本项目目标是基于 Orleans 构建开源通用 MMORPG 服务端框架，按“通用核心 + 可选玩法模块 + 示例游戏”演进。开发顺序与验收条件见 `docs/ROADMAP.md` 和 `docs/BACKLOG.md`，架构及工程要求见 `docs/ARCHITECTURE.md` 和 `docs/ENGINEERING.md`。

功能变更同时维护实现、针对性测试和对应文档；完成后同步待办状态并如实记录未执行的验证。优先保证服务器权威、状态一致性、幂等性和协议兼容。设计模式用于真实变化点和复杂流程，不机械增加接口、转发层或提前拆包。跨 Grain 一致性、持久化/协议演进及模块职责的重大决策记录在 `docs/adr/`。
