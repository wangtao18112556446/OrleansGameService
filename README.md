...
移动按内容版本配置的服务器时间预算核准。示例配置为每秒 6 单位、最多累积 2 秒，因此保留单次 12 单位上限，但连续小步共享预算；切区、重激活和服务器 UTC 回退不会额外刷新距离。`movement_rate_limited` 可等待后使用相同操作号重试，详细规则见[服务器权威移动](docs/gameplay/movement.md)和[实时协议错误码](docs/protocol/errors.md)。
